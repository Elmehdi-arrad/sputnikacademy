<?php 
   ...
   for ($j=0; $j <12 ; $i++) { 
     ...
     $stm = $db->prepare('SELECT * FROM inscrit ORDER BY date_insc ASC');
                   $stm->execute($_POST['id_insc']);

                   foreach($stm->fetchAll() as $row)
                   {

                   
                        $ventes=array($row['id_insc'],$j++); 
                    }
                   }
   ...
   imagettftext($img,20,0,$marge-45,$marge-25,$noir,"./AllCaps.ttf","inscrit"); 
   for($i=0;$i<=10;$i++){ 
   ...
   for($i=0;$i<12;$i++){ 
      ...
      if($type=="Histo"){ 
         imagefilledrectangle($img,$marge+$i*$intX+1,$y-$marge-($ventes[$i]*($y-2*$marge)/100),$marge+$i*$intX+40,$y-$marge-1,$orange); 
         imagefilledrectangle($img,$marge+$i*$intX+1,$y-$marge-($ventes[$i]*($y-2*$marge)/100),$marge+$i*$intX+40,$y-$marge-($ventes[$i]*($y-2*$marge)/100)+5,$noir); 
      } 
      elseif($type=="Courbe"){ 
         imagesetthickness($img,1); 
         if($i<11){ 
            imageline($img,$marge+$i*$intX+1,$y-$marge-($ventes[$i]*($y-2*$marge)/100),$marge+($i+1)*$intX+1,$y-$marge-($ventes[$i+1]*($y-2*$marge)/100),$orange); 
         } 
         imagefilledellipse($img,$marge+$i*$intX+1,$y-$marge-($ventes[$i]*($y-2*$marge)/100),10,10,$noir); 
      } 
      imagettftext($img,12,0,$marge+$i*$intX+8,$y-$marge-($ventes[$i]*($y-2*$marge)/100)-10,$orange,"./AllCaps.ttf",$ventes[$i]); 
   } 
