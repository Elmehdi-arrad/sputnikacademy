...
if(isset($_POST['submit'])){
	...
	$stm = $db->prepare('UPDATE branche SET Nom_br=?  WHERE id_br=?');

 $stm->execute([$_POST['Nom_br'], $_GET['id']]);
	...
	<!-- start: Header -->
		...
		<div id="content" class="span10">
			<div class="row-fluid sortable">		
				...
				$stm = $db->prepare('select  Nom_br  from branche WHERE 	id_br="'.$_GET['id'].'"');
						    $stm->execute();
						    foreach($stm->fetchAll() as $row)
						    { ?>

						<div class="styled-input">
							<input type="text" placeholder="Nom Branche " name="Nom_br" value="<?php echo $row['Nom_br']; ?> "  required>
						</div>
